...
**解决方案**：
NovelGrok使用AI自动识别并统一角色的所有称呼，确保数据准确完整。
...
4. **手动管理接口**
   ```bash
   # 查看角色别名
   GET /api/projects/{title}/characters/{name}/aliases
   
   # 添加别名
   POST /api/projects/{title}/characters/{name}/aliases
   
   # 删除别名
   DELETE /api/projects/{title}/characters/{name}/aliases/{alias}
   
   # 合并重复角色
   POST /api/projects/{title}/characters/merge
   ```
...
**文档**：
- 系统设计：[CHARACTER_ALIAS_SYSTEM.md](CHARACTER_ALIAS_SYSTEM.md)
- 实现总结：[IMPLEMENTATION_ALIAS_SYSTEM.md](IMPLEMENTATION_ALIAS_SYSTEM.md)
- 快速开始：[ALIAS_QUICK_START.md](ALIAS_QUICK_START.md)
- 测试指南：[ALIAS_TEST_GUIDE.md](ALIAS_TEST_GUIDE.md)
